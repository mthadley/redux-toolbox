...
describe(
  ...
  () => {
    it(
      'should call all reducers passed',
      () => {
        const state = {};
...
describe(
  ...
  () => {
    ...
    it(
      'should call the matching action handler',
      () => {
    ...
    it(
      'should return an initial state if not passed one',
      () => {
        const initialState = {};
...
describe(
  ...
  () => {
    it(
      'should return an object of three actionTypes',
      () => {
        const actionTypes = Util.createActionTypes('foo', 'test');

        Object.keys(actionTypes).forEach(
          type => expect(actionTypes[type]).toContain('FOO_TEST')
        );
      }
    );

    it(
      'should genericize action keys',
      () => {
        const actionTypes = Util.createActionTypes('foo', 'test', true);

        Object.keys(actionTypes).forEach(
          type => expect(type).not.toContain('TEST')
        );
      }
    );
  }
